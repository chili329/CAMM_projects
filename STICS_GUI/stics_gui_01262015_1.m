...
% Last Modified by GUIDE v2.5 02-Mar-2015 11:40:15
...
handles.img_file = bfopen();
handles.seri = handles.img_file{1,1};

[xp yp] = size(handles.seri{1,1});
%HERE
%read no. of channels from meta data
%use lsm_read
total_ch = 2;
ch = 1;
tp = size(handles.seri,1)/total_ch;
handles.image_data = zeros(xp,yp,tp);

for i = 1:tp
    if total_ch == 2
    handles.image_data(:,:,i) = handles.seri{i*total_ch-ch,1};
    elseif total_ch ==1
        handles.image_data(:,:,i) = handles.seri{i,1};
    end
end
...
frames = size(handles.image_data,3);
...
imagesc(handles.image_data(:,:,fra))
...
simu_data = simul8tr(256,256,100,[den1 den2],'none',[0 0],[1 1],0.1,0.1,'g',0.3,0,12,[diff1 diff2],[flowX1 flowX2],[flowY1 flowY2],[0 0],countingNoise,backgroundNoise);
...
%HERE IMMOBILE REMOVAL NOT WORKING PROPERLY
if immobile == 1
