...
% pair is the nth pair of a lag time
       ...
       for tau = 0:upperTauLimit-1
           ...
           for pair=1:(size(imgser,3)-tau)
               ...
               %HERE: after immobile removal, use what to normalize?
               mean1 = mean(mean(imgser(:,:,pair)));
               mean2 = mean(mean(imgser(:,:,pair+tau)));
               lagcorr(:,:,pair) = lagcorr(:,:,pair)/(mean1*mean2*size(imgser,1)*size(imgser,2))-1;
           end
